...
public class RPGBThirdPersonController : MonoBehaviour
{
	...
	private void UpdateHorizontalSpeed()
	{
		...
		UpdateAcceleration();
		UpdateDecceleration();
		float acceleration = _hasMovementInput ? MovementSettings.currentAcceleration : MovementSettings.currentDecceleration;
		
		float speedMod = currentSpeed / MovementSettings.DefaultMovementSpeed;
		ControllerEssentials.anim.SetFloat(moveSpeedModifier, Mathf.Lerp(ControllerEssentials.anim.GetFloat(moveSpeedModifier), speedMod, Time.deltaTime * (acceleration/MovementSettings.AnimationSpeedSyncingSpeed)));
	...
	private void UpdateAcceleration()
	{
		MovementSettings.currentAcceleration = Mathf.Lerp(
			MovementSettings.currentAcceleration, isSprinting ? MovementSettings.SprintAcceleration : 
				MovementSettings.Acceleration, Time.deltaTime * MovementSettings.AccelerationLerpSpeed);
	}
	private void UpdateDecceleration()
	{
		MovementSettings.currentDecceleration = Mathf.Lerp(
			MovementSettings.currentDecceleration, isSprinting ? MovementSettings.SprintDecceleration : 
				MovementSettings.Decceleration, Time.deltaTime * MovementSettings.DeccelerationLerpSpeed);
	}
