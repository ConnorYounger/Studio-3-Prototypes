...
namespace BLINK.RPGBuilder.AI
{
    public class RPGBAIAgent : MonoBehaviour
    {
        ...
        public void InitKnockback(float knockbackDistance, float knockbackSpeed, Transform attacker)
        {
            ...
            thisAgent.speed = knockbackSpeed;
            thisAgent.acceleration = knockbackSpeed;
        ...
        private void HandleMobRoaming()
        {
            ...
            if (Time.time >= nextRoam && curMovementState == AGENT_MOVEMENT_STATES.idle)
            {
                ...
                Vector3 newRoamDestination;
                newRoamDestination = thisCombatInfo.spawnerREF != null
                    ? new Vector3(
                        thisCombatInfo.spawnerREF.transform.position.x + Random.Range(-roamRange,
                            roamRange), transform.position.y,
                        thisCombatInfo.spawnerREF.transform.position.z + Random.Range(-roamRange,
                            roamRange))
                    : new Vector3(
