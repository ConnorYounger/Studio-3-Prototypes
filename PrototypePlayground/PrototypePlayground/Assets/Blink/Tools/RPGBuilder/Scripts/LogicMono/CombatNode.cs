...
namespace BLINK.RPGBuilder.LogicMono
{
    public class CombatNode : MonoBehaviour, IPlayerInteractable
    {
        ...
        private void checkIsLeaping()
        {
            ...
            if (playerControllerEssentials.IsInMotionWithoutProgress(0.1f))
            {
                ResetLeap();
                return;
            }

            if (Vector3.Distance(transform.position, LeapEndPOS) < 0.05f)
            {
                ResetLeap();
            }
        }
        ...
        public void InitCasting(RPGAbility thisAbility, int abSlotIndex, RPGAbility.RPGAbilityRankData rankREF)
        {
            ...
            if (rankREF.startCDOnActivate)
            {
                ...
                if(this == CombatManager.playerCombatNode && currentAbilityCastedCurRank.isGCD)CombatManager.Instance.StartGCD();
            }
        ...
        public void DEATH()
        {
            ...
            switch (nodeType)
            {
                case COMBAT_NODE_TYPE.mob:
                {
                    ...
                    if(spawnerREF!=null)spawnerREF.StartCoroutine(spawnerREF.ExecuteSpawner(respawnTime));
